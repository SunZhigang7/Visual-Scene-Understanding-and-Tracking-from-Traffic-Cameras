...
class CoarseRegressor(nn.Module):
    ...
    def __init__(self, n1=20):
        ...
        self.A1 = nn.Sequential(*list(
            list(models.vgg16_bn(
                pretrained=True).children())[0].children())[:7])
        self.A2 = nn.Sequential(*list(
            list(models.vgg16_bn(
                pretrained=True).children())[0].children())[7:14])
        self.A3 = nn.Sequential(*list(
            list(models.vgg16_bn(
                pretrained=True).children())[0].children())[14:24])
        self.A4 = nn.Sequential(*list(
            list(models.vgg16_bn(
                pretrained=True).children())[0].children())[24:34])
        self.A5 = nn.Sequential(*list(
            list(models.vgg16_bn(
                pretrained=True).children())[0].children())[34:])
        # Coarse Regressors
        self.A6 = nn.Sequential(nn.Conv2d(512, 512, 1, padding=0),
                                nn.BatchNorm2d(512), nn.ReLU())
        self.A6to7 = nn.Sequential(nn.Conv2d(512, self.N1 + 1, 1, padding=0),
                                   nn.BatchNorm2d(self.N1 + 1), nn.ReLU())
        self.A3to7 = nn.Sequential(nn.Conv2d(256, self.N1 + 1, 1, padding=0),
                                   nn.BatchNorm2d(self.N1 + 1), nn.ReLU())
        self.A4to7 = nn.Sequential(nn.Conv2d(512, self.N1 + 1, 1, padding=0),
                                   nn.BatchNorm2d(self.N1 + 1), nn.ReLU())
    ...
    def forward(self, x):
        ...
        return self.Up(
            self.Up(
                self.Up(self.A6to7(self.A6(self.A5(res2)))) +
                self.A4to7(res2)) + self.A3to7(x))
...
class FineRegressor(nn.Module):
    ...
    def __init__(self, n2=20):
        ...
        self.L1 = nn.Sequential(nn.Conv2d(24, 64, 7), nn.BatchNorm2d(64),
                                nn.ReLU())
        self.HR1 = nn.Sequential(nn.Conv2d(64, 64, 7), nn.BatchNorm2d(64),
                                 nn.ReLU(), nn.Conv2d(64, 128, 5),
                                 nn.BatchNorm2d(128), nn.ReLU(),
                                 nn.Conv2d(128, 256, 1), nn.BatchNorm2d(256),
                                 nn.ReLU(), nn.ConvTranspose2d(256, 128, 5),
                                 nn.BatchNorm2d(128), nn.ReLU(),
                                 nn.ConvTranspose2d(128, 64, 7),
                                 nn.BatchNorm2d(64), nn.ReLU())
        self.res1 = nn.Sequential(nn.Conv2d(64, 64, 1), nn.BatchNorm2d(64),
                                  nn.ReLU())
        self.L2 = nn.Sequential(nn.ConvTranspose2d(64, self.N2 + 1, 7),
                                nn.BatchNorm2d(self.N2 + 1), nn.ReLU(),
                                nn.Conv2d(self.N2 + 1, 64, 7),
                                nn.BatchNorm2d(64), nn.ReLU())
        self.L3 = nn.Sequential(nn.Conv2d(64, 64, 7), nn.BatchNorm2d(64),
                                nn.ReLU(), nn.Conv2d(64, 128, 5),
                                nn.BatchNorm2d(128), nn.ReLU(),
                                nn.Conv2d(128, 256, 1), nn.BatchNorm2d(256),
                                nn.ReLU())
        self.res2 = nn.Sequential(nn.Conv2d(64, 64, 1), nn.BatchNorm2d(64),
                                  nn.ReLU())
        self.L4 = nn.Sequential(nn.ConvTranspose2d(256, 128, 5),
                                nn.BatchNorm2d(128), nn.ReLU(),
                                nn.ConvTranspose2d(128, 64, 7),
                                nn.BatchNorm2d(64), nn.ReLU())
        self.L5 = nn.Sequential(nn.ConvTranspose2d(64, self.N2 + 1, 7),
                                nn.BatchNorm2d(self.N2 + 1), nn.ReLU())
        self.pose_branch1 = nn.Sequential(nn.Conv2d(256, 128, 7),
                                          nn.BatchNorm2d(128), nn.ReLU(),
                                          nn.Conv2d(128, 64, 7),
                                          nn.BatchNorm2d(64), nn.ReLU())
        self.pose_branch2 = nn.Sequential(nn.Conv2d(64, 32, 7),
                                          nn.BatchNorm2d(32), nn.ReLU())
        self.FC = nn.Sequential(nn.Linear(2048, 256, bias=True),
                                nn.Dropout(0.5), nn.Linear(256, 8, bias=True))
